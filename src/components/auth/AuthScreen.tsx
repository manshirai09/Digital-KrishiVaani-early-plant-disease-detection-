...
export const AuthScreen: React.FC<AuthScreenProps> = ({
  ...
  const handleGoogleSignIn = async () => {
    ...
    setLoading(true);
    try {
      const user = await FirestoreService.signInWithGoogle();
      setSuccessMsg(`Authenticated via Google Firebase! Welcome ${user.name}`);
      setTimeout(() => {
        onSuccess(user);
      }, 300);
    } catch (err: any) {
        ...
        <div className="p-6">
          ...
          <div className="mb-4">
            <button
              ...
              disabled={loading}
              className="w-full py-2.5 px-4 bg-white hover:bg-slate-50 active:bg-slate-100 border border-slate-300 rounded-2xl text-xs font-bold text-slate-800 shadow-xs flex items-center justify-center gap-2.5 transition-all cursor-pointer"
            >
